...
def extracthumanpose(nombrevideo, pintaresqueleto = False, device= 'cuda'):
    ...
    nombrearchivocsv = 'DSImagenesBBoxesTiemposPuntuacion' + nombrevideo + '.csv'
    data = pd.read_csv(nombrearchivocsv) 
    df = pd.DataFrame(data)
    puntos0 = []
    for x in tqdm(df.index):
        image = cv2.imread(path + '/imagenesjugadores/' + df['p0filename'][x], cv2.IMREAD_COLOR)
    ...
    puntos1 = []
    for x in tqdm(df.index):
        image = cv2.imread(path + '/imagenesjugadores/' + df['p1filename'][x], cv2.IMREAD_COLOR)
        joints = modelhuman.predict(image)
...
def extracthumanposewithoutimages(nombrevideo, pintaresqueleto = False, device= 'cuda'):
    modelhuman = SimpleHRNet(48, 17, "./weights/pose_hrnet_w48_384x288.pth", multiperson = False, return_bounding_boxes = False, device = device)
    nombrearchivocsv = 'DSImagenesBBoxesTiemposPuntuacion' + nombrevideo + '.csv'
    data = pd.read_csv(nombrearchivocsv) 
    df = pd.DataFrame(data)
    puntos0 = []
    for x in tqdm(df.index):
        photofilename = path + '/imagenesjuegoono/imagenesjuego/' + df['Filename'][x]
