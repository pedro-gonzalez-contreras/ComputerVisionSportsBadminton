...
def videoimagesgame(nombrevideo):
    ...
    for filename in df['Filename']:
        photofilename = path +'/imagenesjuegoono/imagenesjuego/' + filename
...
def draw_boxes(filename, v_boxes, v_labels, v_scores):
	# load the image
	photofilename2 = path + '/imagenesjuegoono/imagenesjuego/' + filename
...
# Entra imagen sin modificar y la imagen modificada y sale un dataset con la BBoxes de los dos jugadores

def prediccionYOLO(filename, imagenmodificada,draw = True, threshold = 0.5):
...
def pintarBBoxes(resultados,filename):
    ...
    for s in resultados.index:
        if s <= 3:
            ...
            filename2 =  path +  '/imagenesjugadores/'  + ('jugador%d' % count1) + filename ;count1 +=1
...
def yolofromvideo(nombrevideo,pintarBboxesjug = False,draw = False, threshold=0.5):
    ...
    # and associated probabilities
        crop3 = sacarimagenmodificada(frame)
...
def generatebigdataset(nombrevideo,weightpista=861, heightpista=471):
    ...
    for x in dfgrande.index:
        if (dfgrande.p0xmax[x] - dfgrande.p0xmin[x])*(dfgrande.p0ymax[x] - dfgrande.p0ymin[x]) >= weightpista*heightpista:
            ...
            if os.path.isfile(path + '/imagenesjugadores/' +dfgrande.p0filename[x]): 
                shutil.move(path + '/imagenesjugadores/' +dfgrande.p0filename[x], path + '/' + dfgrande.p0filename[x])
                
            old_file_name1 = path + '/imagenesjugadores/' + dfgrande.p1filename[x]
            new_file_name1 = path + '/imagenesjugadores/' + dfgrande.p0filename[x]
            old_file_name2 = path + '/imagenesjugadores/' + dfgrande.p2filename[x]
            new_file_name2 = path + '/imagenesjugadores/' + dfgrande.p1filename[x]
    ...
    for x in dfgrande.index:
        if (dfgrande.p1xmax[x] - dfgrande.p1xmin[x])*(dfgrande.p1ymax[x] - dfgrande.p1ymin[x]) >= weightpista*heightpista:
            ...
            dfgrande.p2ymin[x] = np.nan
            dfgrande.p2ymax[x] = np.nan
            if os.path.isfile(path + '/imagenesjugadores/' +dfgrande.p1filename[x]): 
                shutil.move(path + '/imagenesjugadores/' +dfgrande.p1filename[x],path +'/'+dfgrande.p1filename[x])
            old_file_name3 = path + '/imagenesjugadores/' + dfgrande.p2filename[x]
            new_file_name3 = path + '/imagenesjugadores/' + dfgrande.p1filename[x]
            if os.path.isfile(old_file_name2):
                os.rename(old_file_name3, new_file_name3)

    nombrearchivodatoscsv = 'datospartido' + nombrevideo  + '.csv'
    data2 = pd.read_csv(nombrearchivodatoscsv) 
    dfdatos = pd.DataFrame(data2)
    ...
    for x in dfgrande.index:
        for y in dfdatos.index: 
            if (dfgrande["segundo"][x] >= dfdatos['t_0seg'][y]) and (dfgrande["segundo"][x] <= dfdatos['t_finalseg'][y]):
                dfgrande['p_saca'][x] = dfdatos['p_saca'][y]
                dfgrande['p_win'][x] = dfdatos['p_win'][y]
                dfgrande['score_1'][x] = dfdatos['score_1'][y]
                dfgrande['score_2'][x] = dfdatos['score_2'][y]

    score2 = dfgrande['score_2']
    score1 = dfgrande['score_1']

    score2_df = score2.str.split(expand= True)
    score1_df = score1.str.split(expand= True)

    for i in range(0, len(score2_df.columns)):
        score2_df.iloc[:,i] = pd.to_numeric(score2_df.iloc[:,i], errors='ignore')
        # errors='ignore' lets strings remain as 'non-null objects'
    for i in range(0, len(score1_df.columns)):
        score1_df.iloc[:,i] = pd.to_numeric(score1_df.iloc[:,i], errors='ignore')
        # errors='ignore' lets strings remain as 'non-null objects'

    score2_df.columns = ['setp2','pointsp2']
    score1_df.columns = ['setp1','pointsp1']

    dfgrande= pd.concat([dfgrande,score1_df,score2_df],axis=1)

    dfgrande= dfgrande.drop(['score_1','score_2'], axis = 1)

    nombrearchivocsv = 'Golpes' + nombrevideo + '.csv'
    data2 = pd.read_csv(nombrearchivocsv, header = None) 
    dfgolpeos = pd.DataFrame(data2)


    dfgrande["golpe"]= 'n'
    for x in dfgrande.index:
        for y in dfgolpeos.index:
              if (dfgrande["segundo"][x] >= dfgolpeos[3][y]) and (dfgrande["segundo"][x] < dfgolpeos[5][y]):
                    dfgrande["golpe"][x]  = dfgolpeos[8][y]
                    break
...
def rellenarBboxes(nombrevideo):
    ...
    for x in df.index:
        if os.path.isfile(path + '/imagenesjugadores/' + df.p1filename[x]):
            pass
        else:
            imarray = np.random.rand(100, 100, 3) * 255
            cv2.imwrite(path + '/imagenesjugadores/' + df.p1filename[x], imarray)

    for x in df.index:
        if os.path.isfile(path + '/imagenesjugadores/' + df.p0filename[x]):
            pass
        else:
            imarray = np.random.rand(100, 100, 3) * 255
            cv2.imwrite(path + '/imagenesjugadores/' + df.p0filename[x], imarray)
